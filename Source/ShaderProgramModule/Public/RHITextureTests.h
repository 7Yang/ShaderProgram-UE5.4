...
public:
			...
			auto VerifyMip = [&](void* Ptr, uint32 MipWidth, uint32 MipHeight, uint32 Width, uint32 Height, uint32 CurrentMipIndex, uint32 CurrentSliceIndex, bool bShouldBeZero)
			{
				...
				for (uint32 Y = 0; Y < MipHeight; ++Y)
				{
					...
					for (uint32 X = 0; X < MipWidth; ++X)
					{
						...
						if (bShouldBeZero)
						{
							if (!IsZeroMem(Pixel, NumTestBytes)) return false;
						}
						else
						{
							if (FMemory::Memcmp(Pixel, TestValue, NumTestBytes) != 0) return false;
						}
					}
				}
	...
	static bool Test_RHIClearUAV_Texture2D_WithParams(FRHICommandListImmediate& RHICmdList, uint32 NumMips, uint32 NumSlices, uint32 Width, uint32 Height, EPixelFormat Format, const ValueType& ClearValue, void(FRHIComputeCommandList::* ClearPtr)(FRHIUnorderedAccessView*, ValueType const&), const uint8(&TestValue)[NumTestBytes])
	{
		...
		{
			const FRHITextureCreateDesc Desc = FRHITextureCreateDesc(*TestName, (NumSlices == 1) ? ETextureDimension::Texture2D : ETextureDimension::Texture2DArray).SetFormat(Format).SetExtent(Width, Height).SetArraySize(NumSlices).SetNumMips(NumMips).SetFlags(ETextureCreateFlags::UAV | ETextureCreateFlags::ShaderResource);
	...
	static bool Test_RHIClearUAV_Texture3D_WithParams(FRHICommandListImmediate& RHICmdList, uint32 NumMips, uint32 Width, uint32 Height, uint32 Depth, EPixelFormat Format, const ValueType& ClearValue, void(FRHIComputeCommandList::* ClearPtr)(FRHIUnorderedAccessView*, ValueType const&), const uint8(&TestValue)[NumTestBytes])
	{
		...
		{
			const FRHITextureCreateDesc Desc = FRHITextureCreateDesc::Create3D(*TestName, Width, Height, Depth, Format).SetNumMips(NumMips).SetFlags(ETextureCreateFlags::UAV | ETextureCreateFlags::ShaderResource);
	...
	static bool Test_RHIFormat_WithParams(FRHICommandListImmediate& RHICmdList, EPixelFormat ResourceFormat, EPixelFormat SRVFormat, EPixelFormat UAVFormat, ETextureCreateFlags Flags)
	{
		...
		{
			const FRHITextureCreateDesc Desc = FRHITextureCreateDesc::Create2D(*TestName, Width, Height, ResourceFormat).SetFlags(Flags);
			FTextureRHIRef Texture = RHICreateTexture(Desc);
...
#undef RUN_TEST_ON_FORMAT
	...
	static bool Test_RHICopyTexture(FRHICommandListImmediate& RHICmdList)
	{
...

		{
			// Copy to identical texture
			{
				...
				const FRHITextureCreateDesc SourceDesc = FRHITextureCreateDesc::Create2D(TestName).SetExtent(Extent.X, Extent.Y).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopySrc);
				...
				const FRHITextureCreateDesc DestDesc = FRHITextureCreateDesc::Create2D(TestName).SetExtent(Extent.X, Extent.Y).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopyDest);
			...
			const auto CopyTextureSrgbLambda = [&](const TCHAR* TestName, bool bSourceSrgb, bool bDestSrgb)
			{
				...
				const FRHITextureCreateDesc SourceDesc = FRHITextureCreateDesc::Create2D(TestName).SetExtent(Extent.X, Extent.Y).SetFormat(PF_B8G8R8A8).SetFlags(bSourceSrgb ? ETextureCreateFlags::SRGB : ETextureCreateFlags::None).SetInitialState(ERHIAccess::CopySrc);
				FTextureRHIRef SourceTexture = RHICreateTexture(SourceDesc);
				...
				const FRHITextureCreateDesc DestDesc = FRHITextureCreateDesc::Create2D(TestName).SetExtent(Extent.X, Extent.Y).SetFormat(PF_B8G8R8A8).SetFlags(bDestSrgb ? ETextureCreateFlags::SRGB : ETextureCreateFlags::None).SetInitialState(ERHIAccess::CopyDest);
				FTextureRHIRef DestTexture = RHICreateTexture(DestDesc);
				RHICmdList.CopyTexture(SourceTexture, DestTexture, {});
				RHICmdList.Transition(FRHITransitionInfo(DestTexture, ERHIAccess::CopyDest, ERHIAccess::CopySrc));
			...
			const auto CopyTextureMipsLambda = [&](bool bUseExplicitSize)
			{
				...
				const FRHITextureCreateDesc SourceDesc = FRHITextureCreateDesc::Create2D(TEXT("Copy Texture Mips")).SetExtent(Extent.X, Extent.Y).SetNumMips(NumMips).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopySrc);
				FTextureRHIRef SourceTexture = RHICreateTexture(SourceDesc);
				...
				for (int32 MipIndex = 0; MipIndex < NumMips; ++MipIndex)
				{
					...
					const FRHITextureCreateDesc DestDesc = FRHITextureCreateDesc::Create2D(*TestName).SetExtent(Extent.X >> MipIndex, Extent.Y >> MipIndex).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopySrc);
					FTextureRHIRef DestTexture = RHICreateTexture(DestDesc);
			...
			// Copy mip chains
			{
				...
				const FRHITextureCreateDesc SourceDesc = FRHITextureCreateDesc::Create2DArray(TestName).SetExtent(Extent.X, Extent.Y).SetNumMips(NumMips).SetArraySize(NumArraySlices).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopySrc);
				FTextureRHIRef SourceTexture = RHICreateTexture(SourceDesc);
				...
				*/
				...
				const FRHITextureCreateDesc DestDesc = FRHITextureCreateDesc::Create2DArray(TestName).SetExtent(Extent.X, Extent.Y).SetNumMips(NumMips).SetArraySize(NumArraySlices).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopyDest);
				FTextureRHIRef DestTexture = RHICreateTexture(DestDesc);
			...
			// Copy 3D texture
			{
				...
				const FRHITextureCreateDesc SourceDesc = FRHITextureCreateDesc::Create3D(TestName).SetExtent(64, 64).SetDepth(64).SetNumMips(6).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::CopySrc);
				const FRHITextureCreateDesc DestDesc   = FRHITextureCreateDesc(SourceDesc).SetInitialState(ERHIAccess::CopyDest);
	...
	static bool Test_MultipleLockTexture2D(FRHICommandListImmediate& RHICmdList)
	{
		...
		const FRHITextureCreateDesc Desc = FRHITextureCreateDesc::Create2D(TEXT("FRHITextureTests_StagingTexture"), SliceDim.X, SliceDim.Y, Format).SetFlags(ETextureCreateFlags::CPUReadback).SetInitialState(ERHIAccess::CopyDest);
	...
	static bool RunTest_ClearRenderTarget(FRHICommandListImmediate& RHICmdList, EPixelFormat PixelFormat, TArrayView<FVector4f> ClearColors, TArrayView<TBinaryValue> TestValues, uint32 NumChannels, TBinaryValue Tolerance)
	{
		...
		{
			FString ClearTestName      = FString::Printf(TEXT("Test_ClearRenderTargets (%s) (%.5f %.5f %.5f %.5f)"), GPixelFormats[PixelFormat].Name, ClearColors[ClearColorIndex].X, ClearColors[ClearColorIndex].Y, ClearColors[ClearColorIndex].Z, ClearColors[ClearColorIndex].W);
			FRHITextureCreateDesc Desc = FRHITextureCreateDesc(*ClearTestName, ETextureDimension::Texture2D).SetFormat(PixelFormat).SetExtent(32, 32).SetFlags(ETextureCreateFlags::ShaderResource | ETextureCreateFlags::RenderTargetable).SetClearValue(FClearValueBinding(ClearColors[ClearColorIndex])).SetInitialState(ERHIAccess::RTV);
			FTextureRHIRef Texture     = RHICreateTexture(Desc);
			...
			bool bCurrentColorResult = FRHITextureTests::VerifyTextureContents(*ClearTestName, RHICmdList, Texture, [&](void* Ptr, uint32 MipWidth, uint32 MipHeight, uint32 Width, uint32 Height, uint32 CurrentMipIndex, uint32 CurrentSliceIndex)
			{
				for (uint32 Y = 0; Y < MipHeight; ++Y)
				{
					uint8* Row = ((uint8*)Ptr) + (Y * Width * BytesPerPixel);
