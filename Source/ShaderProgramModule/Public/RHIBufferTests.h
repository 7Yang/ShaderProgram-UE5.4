...
private:
...
#if 1
			FFunctionGraphTask::CreateAndDispatchWhenReady([TestLambda, RHICmdListUpload, NumTestsLaunched, NumTestsInTask](ENamedThreads::Type, const FGraphEventRef&)
			{
	...
	static bool RunTest_UAVClear_Buffer(FRHICommandListImmediate& RHICmdList, const FString& TestName, BufferType* BufferRHI, FRHIUnorderedAccessView* WholeResourceUAV, FRHIUnorderedAccessView* OffsetUAV, const ValueType& ClearValue, void(FRHIComputeCommandList::* ClearPtr)(FRHIUnorderedAccessView*, ValueType const&), const uint8(&TestValue)[NumTestBytes])
	{
	...
	static bool RunTest_UAVClear_Buffer(FRHICommandListImmediate& RHICmdList, EBufferUsageFlags BufferFlags, uint32 BufferSize, EPixelFormat Format, uint32 Stride, bool bRawView, const ValueType& ClearValue, void(FRHIComputeCommandList::* ClearPtr)(FRHIUnorderedAccessView*, ValueType const&), const uint8(&TestValue)[NumTestBytes])
	{
		...
		FString TestName = bStructured ? FString::Printf(TEXT("RunTest_UAVClear_Buffer (Structured) - Stride: %d"), Stride) : FString::Printf(TEXT("RunTest_UAVClear_Buffer (%s) - Format: %s, Raw View: %s"), bRaw ? TEXT("Raw") : TEXT("Typed"), GPixelFormats[Format].Name, bRawView ? TEXT("True") : TEXT("False"));
		...
		{
			UAVCreateDesc.SetType(bStructured ? FRHIViewDesc::EBufferType::Structured : FRHIViewDesc::EBufferType::Typed).SetFormat(Format).SetStride(Stride);
			SRVCreateDesc.SetType(bStructured ? FRHIViewDesc::EBufferType::Structured : FRHIViewDesc::EBufferType::Typed).SetFormat(Format).SetStride(Stride);
		}
		...
		UAVCreateDesc.SetOffsetInBytes(TestOffset).SetNumElements(NumElements);
...
public:
	...
	{
		if (!GRHISupportsMultithreadedResources) { return true; }
		...
		ParallelDispatchCommands(RHICmdList, NumBuffersToCreate, [&](FRHICommandList& InRHICmdList, int32 Index, int32 Num)
		{
			...
			switch (Random % 3)
			{
			    case 0: Usage |= EBufferUsageFlags::Static;    break;
			    case 1: Usage |= EBufferUsageFlags::Dynamic;  break;
			    case 2: Usage |= EBufferUsageFlags::Volatile; break;
			}
