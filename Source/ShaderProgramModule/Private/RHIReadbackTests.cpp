...
IMPLEMENT_GLOBAL_SHADER(FFillBufferCS, "/HLSLShaders/Private/TestFillBuffer.usf", "FillBuffers", SF_Compute);
...
IMPLEMENT_GLOBAL_SHADER(FFillTexturesCS, "/HLSLShaders/Private/TestFillBuffer.usf", "FillTextures", SF_Compute);
...
bool FRHIReadbackTests::Test_BufferReadback(FRHICommandListImmediate& RHICmdList)
{
	...
	// Create typed, byte address and structured buffers.
	{
		...
		auto TypedBufferUAVCreateDesc = FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Typed).SetFormat(PF_R32_UINT).SetNumElements(NumElements);
...

	{
		...
		auto ByteAddressBufferUAVCreateDesc = FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Raw);
...

	{
		...
		auto StructuredBufferUAVCreateDesc = FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(sizeof(FStructuredBufferElem)).SetNumElements(NumElements);
...
bool FRHIReadbackTests::Test_TextureReadback(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		const FRHITextureCreateDesc Texture2DDesc = FRHITextureCreateDesc::Create2D(TEXT("Texture2D")).SetFlags(ETextureCreateFlags::UAV).SetExtent(TextureSize.X, TextureSize.Y).SetNumMips(1).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::UAVCompute);

		Texture2D             = RHICreateTexture(Texture2DDesc);
		auto Texture2DUAVDesc = FRHIViewDesc::CreateTextureUAV().SetDimensionFromTexture(Texture2D).SetFormat(PF_R32_UINT);
		Texture2DUAV          = RHICmdList.CreateUnorderedAccessView(Texture2D, Texture2DUAVDesc);
	}

	{
		const FRHITextureCreateDesc Texture3DDesc = FRHITextureCreateDesc::Create3D(TEXT("Texture3D")).SetFlags(ETextureCreateFlags::UAV).SetExtent(TextureSize.X, TextureSize.Y).SetDepth(TextureSize.Z).SetNumMips(1).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::UAVCompute);

		Texture3D             = RHICreateTexture(Texture3DDesc);
		auto Texture3DUAVDesc = FRHIViewDesc::CreateTextureUAV().SetDimensionFromTexture(Texture3D).SetFormat(PF_R32_UINT);
		Texture3DUAV          = RHICmdList.CreateUnorderedAccessView(Texture3D, Texture3DUAVDesc);
	}

	{
		const FRHITextureCreateDesc TextureArrayDesc = FRHITextureCreateDesc::Create2DArray(TEXT("TextureArray")).SetFlags(ETextureCreateFlags::UAV).SetExtent(TextureSize.X, TextureSize.Y).SetArraySize(TextureSize.Z).SetFormat(PF_R32_UINT).SetInitialState(ERHIAccess::UAVCompute);

		TextureArray             = RHICreateTexture(TextureArrayDesc);
		auto TextureArrayUAVDesc = FRHIViewDesc::CreateTextureUAV().SetDimensionFromTexture(TextureArray).SetFormat(PF_R32_UINT);
