...
{
	//if (!GRHIGlobals.ReservedResources.SupportsVolumeTextures) { return true; }
...

	{
		...
		Desc.SetFlags(TexCreate_ShaderResource | TexCreate_ReservedResource | TexCreate_ImmediateCommit).SetExtent(FIntPoint(8, 8)).SetDepth(8).SetNumMips(1).SetFormat(PF_A32B32G32R32F).SetInitialState(ERHIAccess::SRVCompute);
...

	{
		...
		Desc.SetFlags(TexCreate_ShaderResource | TexCreate_ReservedResource | TexCreate_ImmediateCommit).SetExtent(FIntPoint(128, 128)).SetDepth(64).SetNumMips(1).SetFormat(PF_A32B32G32R32F).SetInitialState(ERHIAccess::SRVCompute);
		FTextureRHIRef Texture     = RHICreateTexture(Desc);
	}
...

	{
		...
		Desc.SetFlags(TexCreate_ShaderResource | TexCreate_ReservedResource | TexCreate_ImmediateCommit).SetExtent(FIntPoint(1024, 1024)).SetDepth(8).SetNumMips(1).SetFormat(PF_A32B32G32R32F).SetInitialState(ERHIAccess::SRVCompute);
...

	{
		...
		Desc.SetFlags(TexCreate_ShaderResource | TexCreate_ReservedResource).SetExtent(FIntPoint(2048, 2048)).SetDepth(2048).SetNumMips(1).SetFormat(PF_A32B32G32R32F).SetInitialState(ERHIAccess::SRVCompute);
		FTextureRHIRef Texture     = RHICreateTexture(Desc);
	}
...
{
	//if (!GRHIGlobals.ReservedResources.Supported) { return true; }

	const ETextureCreateFlags CreateFlags   = TexCreate_ShaderResource | TexCreate_ReservedResource | TexCreate_ImmediateCommit;
	const EPixelFormat        TestFormats[] = { PF_DXT1, PF_R8, PF_R32_UINT };
	...
	for (FTestTextureConfig Config : TestConfigs)
	{
		for (EPixelFormat Format : TestFormats)
		{
			...
			Desc.SetFlags(CreateFlags).SetExtent(Config.Extent).SetNumMips(Config.NumMips).SetFormat(Format).SetInitialState(ERHIAccess::SRVCompute);
			FTextureRHIRef Texture = RHICreateTexture(Desc);
		}
	}
...

	{
		...
		Desc.SetFlags(TexCreate_ShaderResource | TexCreate_ReservedResource).SetExtent(FIntPoint(16384, 16384)).SetNumMips(1).SetFormat(PF_A32B32G32R32F).SetInitialState(ERHIAccess::SRVCompute);
		FTextureRHIRef Texture     = RHICreateTexture(Desc);
	}
...
static void CommitBuffer(FRHICommandListImmediate& RHICmdList, FRHIBuffer* Buffer, uint64 CommitSize, ERHIAccess StateBefore, ERHIAccess StateAfter)
{
	...
	FRHITransitionCreateInfo CreateInfo(ERHIPipeline::Graphics, ERHIPipeline::Graphics, ERHITransitionCreateFlags::None, TransitionInfos);
...
{
	return FRHIBufferTests::VerifyBufferContents(TestName, RHICmdList, MakeArrayView(&Buffer, 1), [Ranges](int32 /*BufferIndex*/, void* Ptr, uint32 NumBytes)
	{
...
bool FRHIReservedResourceTests::Test_ReservedResource_CommitBuffer(FRHICommandListImmediate& RHICmdList)
{
	...
	FBufferRHIRef Buffer = RHICmdList.CreateBuffer(BufferSizeInBytes, BUF_ReservedResource | BUF_UnorderedAccess | BUF_ShaderResource | BUF_SourceCopy, 4, ERHIAccess::UAVCompute, CreateInfo);
	FUnorderedAccessViewRHIRef BufferUAV = RHICmdList.CreateUnorderedAccessView(Buffer, FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Typed).SetFormat(PF_R32_UINT));
